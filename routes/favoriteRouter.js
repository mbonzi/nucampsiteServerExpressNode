    ...
    Favorite.findOneAndDelete({ user: req.user._id })
      .then((favorite) => {
        ...
        if (favorite) {
          ...
          res.end("There are no favorites to delete yet.");
        }
      })
      .catch((err) => next(err));
  });
...
favoriteRouter
      ...
      .then((favorite) => {
        if (favorite) {
          if (!favorite.campsites.includes(req.params.campsiteId)) {
            ...
            res.setHeader("Content-Type", "text/plain");
            res.end(
              "That campsite is already listed in favorites. Feel free to add a new one!"
            );
          }
        } else {
          Favorite.create({
  ...
  .delete(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    Favorite.findOne({ user: req.user._id })
      .then((favorite) => {
        if (favorite) {
          const faveToDelete = favorite.campsites.indexOf(
            req.params.campsiteId
          );
          favorite.campsites.splice(faveToDelete, 1);
          favorite
            .save()
            .then((favorite) => {
              console.log("That fave campsite has been deleted.", favorite);
              res.statusCode = 200;
              res.setHeader("Content-Type", "application/json");
              res.json(favorite);
            })
